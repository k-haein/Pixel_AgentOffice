...
## 진급 시스템 (Day 13 — Phase 3)

### 무엇
직원이 활동 누적으로 진급 자격에 도달하면 **캐릭터가 사장(사용자)에게 진급을 요청**하는 모달. 자동 진급이 아니라 승인 방식(ideas/11 §4). 직원별 `promotionMode`(정량/시간/정성/혼합/수동)에 따라 판정.

### 판정 기준 (`shared/promotion.ts`, ideas/11 §2)
- **정량형**: 누적 대화·메모 (사원 5 / 대리 50·메모1 / 과장 150·메모5 / 부장 400·메모15 / 이사 800·메모30 / 사장 2000·메모100)
- **시간형**: 입사 경과 (사원 3일 / 대리 2주 / 과장 1개월 / 부장 3개월 / 이사 6개월 / 사장 1년)
- **정성형**: 받은 칭찬 (사원 1 / 대리 5 / 과장 15 / 부장 40 / 이사 100)
- **혼합형**: 위 3개 중 2개 이상 충족
- **수동(off)**: 자동 제안 안 함
...
### 사용 방법
1. (정량 기준) 직원과 대화를 누적 → 다음 직급 임계 도달
2. 자동으로 진급 요청 모달 등장 — 캐릭터 voice + 현재→희망 직급 + 성과(대화/메모/칭찬/입사일)
3. **😊 진급시키기** → 직급 상승 + 캐릭터 happy / **🤔 다음에** → 닫힘(다음 트리거에 다시 제안)

### 기대 동작 ☐
- [ ] 정량형 직원: 대화 5회 누적 → 알바→사원 진급 요청 모달 자동 등장
- [ ] 모달에 캐릭터 emoji·이름·voice·현재→희망 직급·성과 표시
- [ ] "진급시키기" → 직급 오름(메모 모달 🏆에 반영) + 사무실 캐릭터 happy 6초
- [ ] "다음에" → 모달 닫힘, 직급 유지
- [ ] 수동(off) 모드 직원은 진급 요청 안 뜸
- [ ] backdrop 드래그해도 안 닫힘(mousedown 기준), 외부 클릭 시 닫힘
- [ ] 한 번에 여러 직원 자격이어도 모달은 1개씩

### 알려진 한계
- 회장·레전드 진급은 자동 제안 안 함 (사장까지만 — 임계 미정의)
- 진급 voice는 기본 정중체 1종 — MBTI/성격별 변형은 후속
- 콘페티 등 화려한 연출은 없음 (happy 이모지로 대체). 직급별 외형 변화(명패 색 등)도 후속
- 시간형은 앱 로드 시에만 체크 — 앱 켜둔 채로 시간이 지나면 다음 실행 때 반영
