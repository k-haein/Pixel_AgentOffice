...
>
> 최종 갱신: **2026-05-27 (Day 13)** — 버그 2건 fix(모달 드래그 닫힘 / 설정 모달 중첩) + 🚧 미완·멈춤 레지스터 신설 + 📣 테스터 피드백 표(T1 튜토리얼) + 포트폴리오 **v0.0.0/v0.0.1 버전 분리**(PRD·유저플로우·와이어프레임·코드스냅샷 최신화) + E2E 전반 실행→stale 진단 + **진급/메모리/칭찬 Phase 1~3 완료**(활동 카운터 + 칭찬 👍 + 진급 요청 모달). **진급 기준 재정의**(정량=대화만 50/100/200/400, 자동은 부장까지, 이사·사장은 사장이 직접 임명) + **진급 UI**(채용 tip·메모 진행도/방식변경·직급 팻말·이사 임명) + **오타 점검 룰**(CONVENTIONS §6-1·메모리). **진급 난이도 배율 설정**(설정에서 0.5~3배 — Workflow 적대적 리뷰로 버그 4건 잡아 수정) + **정성 기준 재정의 5/20/50/100**. 사용자 dev 검증 완료. **Phase 4 직원 기억(메모리) 시스템 — 반자동**(app-data.json 저장 + system prompt 자동 주입 + 메모 모달 "🧠 대화에서 기억 정리" 수동 요약 버튼. Workflow 적대적 리뷰로 동시성·데이터 유실 7건 수정) — 레지스터 A 마지막 잔여 해소. 다음: v0.0.2 재빌드(버그fix + Phase 1~4 + 배율 + 메모리) / E2E 재작성 / 튜토리얼(T1) / 맥 .dmg.
...
| **다음 작업** | **v0.0.2 재빌드**(버그fix + Phase 1~4 + 배율 + 메모리 반영). E2E 빈사무실 기준 재작성(promotion.ts 유닛테스트 포함). 튜토리얼(T1). 맥 .dmg(GitHub Actions). 메모리 후속(완전 자동·야간 압축 트리거, MemoryMode 토글 연결). |
...
> 메모리 후속 잔여(멈춘 게 아닌 단순 확장): 완전 자동·야간 압축 트리거, `MemoryMode`(off/manual/ask/auto) 토글을 실제 동작에 연결. 현재는 메모 모달 "🧠 대화에서 기억 정리" 버튼이 실질 트리거.
