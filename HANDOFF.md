...
| **현재 마일스톤** | **M5 시그니처 폴리시 완성** + Day 10~12 폴리시 + Day 12 §3+1(알림 모달·Cody·EXE v0.0.1) + **Day 13: 버그 2건 fix + 미완 레지스터 + 포트폴리오 v0.0.1 + 진급/칭찬 Phase 1~3 + 진급 기준 재정의·UI·직급 팻말·이사 임명 + 진급 난이도 배율(Workflow 리뷰로 버그 4건 수정)·정성 5/20/50/100** |
| **다음 작업** | **Phase 4 메모리 시스템**(LLM이 대화에서 사실 추출→저장→system prompt 주입. 레지스터 A 마지막 잔여, ideas/09 기획). v0.0.2 재빌드(버그fix + Phase 1~3 + 배율 반영). E2E 빈사무실 기준 재작성(promotion.ts 유닛테스트 포함). 튜토리얼(T1). 맥 .dmg(GitHub Actions). |
