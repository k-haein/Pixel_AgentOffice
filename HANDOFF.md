...
>
> 최종 갱신: **2026-05-27 (Day 13)** — 버그 2건 fix(모달 드래그 닫힘 / 설정 모달 중첩) + 🚧 미완·멈춤 레지스터 신설 + 📣 테스터 피드백 표(T1 튜토리얼) + 포트폴리오 **v0.0.0/v0.0.1 버전 분리**(PRD·유저플로우·와이어프레임·코드스냅샷 최신화) + E2E 전반 실행→stale 진단 + **진급/메모리/칭찬 Phase 1~3 완료**(활동 카운터 + 칭찬 👍 + 진급 요청 모달). **진급 기준 재정의**(정량=대화만 50/100/200/400, 자동은 부장까지, 이사·사장은 사장이 직접 임명) + **진급 UI**(채용 tip·메모 진행도/방식변경·직급 팻말·이사 임명) + **오타 점검 룰**(CONVENTIONS §6-1·메모리). 사용자 dev 검증 완료. 다음: 진급 난이도 배율 설정 / Phase 4 메모리 / v0.0.2 재빌드 / E2E 재작성 / 튜토리얼(T1).
...
| **현재 마일스톤** | **M5 시그니처 폴리시 완성** + Day 10~12 폴리시 + Day 12 §3+1(알림 모달·Cody·EXE v0.0.1) + **Day 13: 버그 2건 fix + 미완 레지스터 + 포트폴리오 v0.0.1 + 진급/칭찬 Phase 1~3(활동 카운터·칭찬👍·진급 모달) + 진급 기준 재정의·UI·직급 팻말·이사 임명** |
| **다음 작업** | **진급 난이도 배율 설정**(설정에 배율 하나로 전체 기준 조정 — Settings 필드 + promotion.ts가 settings 받기). Phase 4 메모리 실동작. v0.0.2 재빌드(버그fix + Phase 1~3 반영). E2E 빈사무실 기준 재작성(promotion.ts 유닛테스트 포함). 튜토리얼(T1). 맥 .dmg(GitHub Actions). |
...
10. **진급 기준 재정의 + UI + 팻말 + 이사 임명** (이번 커밋, 사용자 검증 완료) — 정량=대화만(사원50/대리100/과장200/부장400), 자동은 부장까지·이사·사장은 사장이 직접 임명. 채용 tip + 메모 진행도바/진급방식 변경 + 진급모달 기준달성 문구 + 캐릭터 아래 `🏆 직급` 팻말 + 부장→이사 임명 버튼
11. **오타 점검 룰** (이번 커밋) — 한자혼입·깨진글자 반복 지적 → CONVENTIONS §6-1 + 메모리 `feedback_proofread_before_work.md`
...
| # | 날짜 | 피드백 | 분류 | 상태 |
|---|---|---|---|---|
| T1 | 2026-05-26 | **최초 실행 시 기능이 너무 많아 보기 힘듦 → 튜토리얼/온보딩 필요** | UX·온보딩 | 🔲 미착수 |
