...
| **다음 작업** | **v0.0.2 재빌드**(Day 13~14 전부: 버그fix + Phase 1~4 + 배율 + 메모리 + **Day 14 온보딩**). E2E 빈사무실 기준 재작성(promotion.ts 유닛테스트 포함). 맥 .dmg(GitHub Actions). 메모리 후속(완전 자동·야간 압축 트리거). 튜토리얼 후속(상점/설정 트랙 옵션 추가 시 단계 보강). |
